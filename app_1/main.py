...
def compute_dataset_statistics(df: pd.DataFrame) -> list:
    """Computes pure statistical summaries isolating qualitative code IDs."""
    stats_list = []
    for col in df.columns:
        ...
        if col_type == "Numeric":
            mean_val = f"{df[col].mean():,.2f}" if not df[col].empty else "0.00"
            median_val = f"{df[col].median():,.2f}" if not df[col].empty else "0.00"
...
async def clean_dataset_pipeline(session_id: str = Form(...), action: str = Form(...)):
    if session_id not in SESSIONS_DB: return {"error": "Active workspace session expired."}
    ...
    return {
        "success": True, "stats": compute_dataset_statistics(df), "rows": f"{len(df):,}",
...
async def plot_insights_laboratory(session_id: str = Form(...), column: str = Form(...), plot_type: str = Form(...)):
    ...
    try:
        ...
        plt.xticks(rotation=30, ha='right', fontsize=8, color='#475569')
    ...
    try:
        while True:
            ...
            if payload.get("type") == "direct_edit":
                ...
                await websocket.send_json({"type": "message", "content": f"📝 **Slide {target_idx} manual changes saved to state.**", "suggestions": ["Approve Plan & Compile Presentation"], "draft_slides": slides})
                continue
